...
import {
  ...
  getIndices,
  drawModes,
  getUniformUploadFunc,
  getStrokeUniformOptions,
} from './settings';
...
export default class Renderer {
  ...
  constructor(params: IRendererParams) {
    ...
    for (let i = 0, len = programInfos.length; i < len; i++) {
      ...
      this._shaders[info.name] = glutils.createProgramInfo(
        gl,
        info.vss,
        info.fss,
        info.attribParams,
  ...
  renderSprite(sprite: Sprite): void {
    ...
    const transformation = m3.someMultiply(
      this._projectionMat,
      sprite.parentTransform,
      sprite.transform,
      textureScaling,
    );
