...
export function createLinkedVBO(
  ...
  for (let i = 0, len = attrParams.length; i < len; i++) {
    ...
    gl.vertexAttribPointer(loc, param.size, (gl as any)[param.type], false, param.stride, param.offset);
  }
...
export function createLinkedVertexBuffer(
  gl: WebGL2RenderingContext,
  program: WebGLProgram,
  positionAttribName: string,
  coordAttribName: string,
) {
